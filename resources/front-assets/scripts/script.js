(function($) {
    ...
    $(".em-responsive").each(function() {
        if ($(this).attr("start-responsive")) {
            responsiveSlider(
                $(this),
                parseInt($(this).attr("start-responsive"), 10),
                false
            );
        }
    });
    ...
    if ($(".number-input").length) {
        ...
        $(".number-input .minus").on("click", function() {
            ...
            inputSelector.val(function(i, val) {
                if (val > -1) {
                    return parseInt(inputSelector.val(), 10) - 1;
                } else {
                    return val;
                }
            });
    ...
    if ($(".slider-ui").length) {
        $(".slider-ui").each(function() {
            ...
            if (selector.attr("slider-min"))
                slider_min = parseInt(selector.attr("slider-min"), 10);
            if (selector.attr("slider-max"))
                slider_max = parseInt(selector.attr("slider-max"), 10);
            if (selector.attr("slider-min-start"))
                slider_min_start = parseInt(
                    selector.attr("slider-min-start"),
                    10
                );
            if (selector.attr("slider-max-start"))
                slider_max_start = parseInt(
                    selector.attr("slider-max-start"),
                    10
                );
            selector.slider({
                range: true,
                ...
                slide: function(event, ui) {
                    $('.range_value_min[target="#' + iid + '"]').val(
                        ui.values[0]
                    );
                    $('.range_value_max[target="#' + iid + '"]').val(
                        ui.values[1]
                    );
                }
            });
        ...
        $(".range_value_min").on("change", function() {
            ...
            var selector_other = $(
                '.range_value_max[target="' + $(this).attr("target") + '"]'
            );
            ...
            if (!selector_other.hasClass("error")) {
                if (value_this < 0 || value_this > value_other) {
                    ...
                    $($(this).attr("target")).slider("option", "values", [
                        value_this,
                        value_other
                    ]);
                    $(this).val(value_this);
        ...
        $(".range_value_max").on("change", function() {
            ...
            var selector_other = $(
                '.range_value_min[target="' + $(this).attr("target") + '"]'
            );
            var value_other = parseInt(crop_currency(selector_other.val()), 10);
    ...
    function add_class_play_animation(selector) {
        ...
        if (selector.hasClass("go-animation")) {
            if (current_top > window_top) {
                if (
                    current_top - window_top <= window_height / 2 ||
                    window_top + window_height - current_top >= current_height
                ) {
                ...
                if (
                    window_top - current_top - current_height <=
                        window_height / 2 ||
    ...
    function CountBack(secs, iid, mcTimer) {
        ...
        DisplayStr = DisplayFormat.replace(
            /%%D%%/g,
            calcage(secs, 86400, 100000)
        );
        ...
        setTimeout(function() {
            CountBack(secs + CountStepper, iid, mcTimer);
        }, SetTimeOutPeriod);
    }
    ...
    function popup() {
        ...
        var imageUrl = "resources/front-assets/images/popup-newletter.jpg";
        var pwidth = 810;
        var pheight = 500;
    ...
    function animate_number() {
        $(".process-skill .process-bar").each(function() {
            ...
            $(this)
                .children(".percent-text")
                .children("span")
                .animateNumber({ number: x }, 900);
        });
    }
    ...
    function magic_tab() {
        $(".featured-product-tab .magictabs .item").on("click", function() {
            if (!$(this).hasClass("active")) {
                $(".featured-product-tab .magictabs .item").removeClass(
                    "active"
                );
                $(
                    ".featured-product-tab .content-products .mage-magictabs"
                ).removeClass("active");
                $(this).addClass("active");
                $(
    ...
    function slider_in_index() {
        $(
            ".featured-product-tab .content-products .products-grid-rows"
        ).bxSlider({
            moveSlides: 2,
            infiniteLoop: 0,
            maxSlides: 4,
